 ...
 *
 */
public class SecurityInfoDaoDefaultImpl implements SecurityInfoDao {
	 ...
	 * 构造函数
	 */
	public SecurityInfoDaoDefaultImpl() {
		initRefreshThread();
	}
	...
	public void set(String key, String value, long timeout) {
		if(timeout == 0 || timeout <= SecurityInfoDao.NOT_VALUE_EXPIRE)  {
			return;
		}
		dataMap.put(key, value);
		expireMap.put(key, (timeout == SecurityInfoDao.NEVER_EXPIRE) ? (SecurityInfoDao.NEVER_EXPIRE) : (System.currentTimeMillis() + timeout * 1000));
	}

	@Override
	public void update(String key, String value) {
		if(getKeyTimeout(key) == SecurityInfoDao.NOT_VALUE_EXPIRE) {
			return;
		}
		dataMap.put(key, value);
	}
	...
	void clearKeyByTimeout(String key) {
		...
		if(expirationTime != null && expirationTime != SecurityInfoDao.NEVER_EXPIRE && expirationTime < System.currentTimeMillis()) {
			dataMap.remove(key);
			expireMap.remove(key);
		}
	}
	...
	long getKeyTimeout(String key) {
		...
		// 如果被标注为永不过期 
		if(expire == SecurityInfoDao.NEVER_EXPIRE) {
	...
	public List<String> searchData(String prefix, String keyword, int start, int size) {
		return FoxUtil.searchList(expireMap.keySet(), prefix, keyword, start, size);
	}
