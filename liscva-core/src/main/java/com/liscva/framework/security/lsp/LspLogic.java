...
import com.liscva.framework.core.context.model.LiscvaCookie;
...
public class LspLogic {
	 ...
	 */
	public LspLogic(String loginType) {
		this.loginType = loginType;
		// 在 SecurityManager 中记录下此 LspLogic，以便根据 LoginType 进行查找此对象
		SecurityManager.putLspLogic(this);
	}
	 ...
	 * @return 对象自身
	 */
	public LspLogic setLoginType(String loginType){
		this.loginType = loginType;
		SecurityManager.putLspLogic(this);
		return this;
	}
	...
	public String createTokenValue(Object loginId, String device, long timeout, Map<String, Object> extraData) {
		return SecurityInfoStrategy.me.createToken.apply(loginId, loginType);
	}
	...
	public void setTokenValue(String tokenValue){
		setTokenValue(tokenValue, (int)SecurityManager.getConfig().getTimeout());
	}
	...
	public void setTokenValueToStorage(String tokenValue){
		// 1. 将token保存到[存储器]里  
		LiscvaStorage storage = SecurityHolder.getStorage();
		
		// 2. 如果打开了 Token 前缀模式，则拼接上前缀
		String tokenPrefix = getConfig().getTokenPrefix();
		if(FoxUtil.isEmpty(tokenPrefix) == false) {
			storage.set(splicingKeyJustCreatedSave(), tokenPrefix + CoreConsts.TOKEN_CONNECTOR_CHAT + tokenValue);	
		} else {
			storage.set(splicingKeyJustCreatedSave(), tokenValue);	
		}
		
		// 3. 写入 (无前缀) 
		storage.set(CoreConsts.JUST_CREATED_NOT_PREFIX, tokenValue);  
	}
	...
	public void setTokenValueToCookie(String tokenValue, int cookieTimeout){
		SecurityCookieConfig cfg = getConfig().getCookie();
		LiscvaCookie cookie = new LiscvaCookie()
				...
				;
		SecurityHolder.getResponse().addCookie(cookie);
	}
	...
	public String getTokenValue(){
		...
		if(FoxUtil.isEmpty(tokenPrefix) == false) {
			...
			if(FoxUtil.isEmpty(tokenValue) || tokenValue.startsWith(tokenPrefix + CoreConsts.TOKEN_CONNECTOR_CHAT) == false) {
				tokenValue = null;
			} else {
				// 则裁剪掉前缀 
				tokenValue = tokenValue.substring(tokenPrefix.length() + CoreConsts.TOKEN_CONNECTOR_CHAT.length());
			}
		}
	...
	public String getTokenValueNotCut(){
		// 0. 获取相应对象 
		LiscvaStorage storage = SecurityHolder.getStorage();
		LiscvaRequest request = SecurityHolder.getRequest();
	...
	public void login(Object id) {
		login(id, new SecurityLoginModel());
	}
	...
	public void login(Object id, String device) {
		login(id, new SecurityLoginModel().setDevice(device));
	}
	...
	public void login(Object id, boolean isLastingCookie) {
		login(id, new SecurityLoginModel().setIsLastingCookie(isLastingCookie));
	}
	 ...
	 */
	public void login(Object id, SecurityLoginModel loginModel) {
		// 1、创建会话 
	...
	public String createLoginSession(Object id) {
		return createLoginSession(id, new SecurityLoginModel());
	}
	 ...
	 * @return 返回会话令牌 
	 */
	public String createLoginSession(Object id, SecurityLoginModel loginModel) {
		
		CoreException.throwByNull(id, "账号id不能为空");
		...
		SecurityInfoSession session = getSessionByLoginId(id, true);
		...
		// $$ 通知监听器，账号xxx 登录成功 
		SecurityManager.getSecurityListener().doLogin(loginType, id, loginModel);
	...
	public void logout() {
		...
		String tokenValue = getTokenValue();
 		...
 		// 从当前 [storage存储器] 里删除 
 		SecurityHolder.getStorage().delete(splicingKeyJustCreatedSave());
	...
	public void logout(Object loginId, String device) {
		clearTokenCommonMethod(loginId, device, tokenValue -> {
			...
			SecurityManager.getSecurityListener().doLogout(loginType, loginId, tokenValue);
		}, true);
	}
	...
	public void logoutByTokenValue(String tokenValue) {
		...
		// 4. 清理User-Session上的token签名 & 尝试注销User-Session 
 	 	SecurityInfoSession session = getSessionByLoginId(loginId, false);
 	 	if(session != null) {
	...
	public void kickout(Object loginId, String device) {
		clearTokenCommonMethod(loginId, device, tokenValue -> {
			...
			updateTokenToIdMapping(tokenValue, NotLoginException.KICK_OUT);
	 		SecurityManager.getSecurityListener().doKickout(loginType, loginId, tokenValue);
		}, true);
	}
	...
	public void replaced(Object loginId, String device) {
		clearTokenCommonMethod(loginId, device, tokenValue -> {
			...
			updateTokenToIdMapping(tokenValue, NotLoginException.BE_REPLACED);
	 		SecurityManager.getSecurityListener().doReplaced(loginType, loginId, tokenValue);
		}, false);
	}
	...
	public Object getLoginIdDefaultNull() {
		...
		String tokenValue = getTokenValue();
 		...
 		// 如果已经[临时过期] 
 		if(getTokenActivityTimeoutByToken(tokenValue) == SecurityInfoDao.NOT_VALUE_EXPIRE) {
 			return null;
 		}
	 ...
	 */
 	...
 	public String getLoginIdNotHandle(String tokenValue) {
 		return getSecurityInfoDao().get(splicingKeyTokenValue(tokenValue));
 	}
	...
	public void deleteTokenToIdMapping(String tokenValue) {
		getSecurityInfoDao().delete(splicingKeyTokenValue(tokenValue));
	}
	/**
	 * 更改 Token 指向的 账号Id 值 
	 * @param tokenValue token值 
	...
	public void updateTokenToIdMapping(String tokenValue, Object loginId) {
		CoreException.throwBy(FoxUtil.isEmpty(loginId), "LoginId 不能为空");
		getSecurityInfoDao().update(splicingKeyTokenValue(tokenValue), loginId.toString());
	}
	/**
	 * 存储 Token-Id 映射 
	 * @param tokenValue token值 
	...
	public void saveTokenToIdMapping(String tokenValue, Object loginId, long timeout) {
		getSecurityInfoDao().set(splicingKeyTokenValue(tokenValue), String.valueOf(loginId), timeout);
	}
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getSessionBySessionId(String sessionId, boolean isCreate) {
		SecurityInfoSession session = getSecurityInfoDao().getSession(sessionId);
		if(session == null && isCreate) {
			session = SecurityInfoStrategy.me.createSession.apply(sessionId);
			getSecurityInfoDao().setSession(session, getConfig().getTimeout());
		}
		return session;
	}
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getSessionBySessionId(String sessionId) {
	 ...
	 * @return Session对象
	 */
	public SecurityInfoSession getSessionByLoginId(Object loginId, boolean isCreate) {
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getSessionByLoginId(Object loginId) {
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getSession(boolean isCreate) {
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getSession() {
	 ...
	 * @return session对象
	 */
	public SecurityInfoSession getTokenSessionByToken(String tokenValue, boolean isCreate) {
	 ...
	 * @return Session对象  
	 */
	public SecurityInfoSession getTokenSessionByToken(String tokenValue) {
	 ...
	 * @return Session对象  
	 */
	public SecurityInfoSession getTokenSession(boolean isCreate) {
		// 如果配置了需要校验登录状态，则验证一下
		if(getConfig().getTokenSessionCheckLogin()) {
			...
			if(tokenValue == null || Objects.equals(tokenValue, "")) {
				...
				setLastActivityToNow(tokenValue);  
				// 在当前会话写入这个tokenValue 
				int cookieTimeout = (int)(getConfig().getTimeout() == SecurityInfoDao.NEVER_EXPIRE ? Integer.MAX_VALUE : getConfig().getTimeout());
	 ...
	 * @return Session对象 
	 */
	public SecurityInfoSession getTokenSession() {
	...
	public void deleteTokenSession(String tokenValue) {
		getSecurityInfoDao().delete(splicingKeyTokenSession(tokenValue));
	}
	...
	/**
 	...
 	protected void setLastActivityToNow(String tokenValue) {
 		...
 		// 将[最后操作时间]标记为当前时间戳 
 		getSecurityInfoDao().set(splicingKeyLastActivityTime(tokenValue), String.valueOf(System.currentTimeMillis()), getConfig().getTimeout());
 	}
 	...
 	protected void clearLastActivity(String tokenValue) {
 		...
 		// 删除[最后操作时间]
 		getSecurityInfoDao().delete(splicingKeyLastActivityTime(tokenValue));
 		// 清除标记 
 		SecurityHolder.getStorage().delete(CoreConsts.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY);
 	}
 	...
 	public void checkActivityTimeout(String tokenValue) {
 		...
 		LiscvaStorage storage = SecurityHolder.getStorage();
 		if(storage.get(CoreConsts.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY) != null) {
 			return;
 		}
 		// ------------ 验证是否已经 [临时过期] 
 		// 获取 [临时剩余时间]
 		...
 		// -2 代表已过期，抛出异常 
 		if(timeout == SecurityInfoDao.NOT_VALUE_EXPIRE) {
 			throw NotLoginException.newInstance(loginType, NotLoginException.TOKEN_TIMEOUT, tokenValue);
 		}
 		// --- 至此，验证已通过 

 		// 打上[检查标记]，标记一下当前请求已经通过验证，避免一次请求多次验证，造成不必要的性能消耗 
 		storage.set(CoreConsts.TOKEN_ACTIVITY_TIMEOUT_CHECKED_KEY, true);
 	}
 	...
 	public void updateLastActivityToNow(String tokenValue) {
 		...
 		getSecurityInfoDao().update(splicingKeyLastActivityTime(tokenValue), String.valueOf(System.currentTimeMillis()));
 	}
	...
	// ------------------- 过期时间相关 -------------------  
 	...
 	public long getTokenTimeout() {
 		return getSecurityInfoDao().getTimeout(splicingKeyTokenValue(getTokenValue()));
 	}
 	...
 	public long getTokenTimeoutByLoginId(Object loginId) {
 		return getSecurityInfoDao().getTimeout(splicingKeyTokenValue(getTokenValueByLoginId(loginId)));
 	}
 	...
 	public long getSessionTimeoutByLoginId(Object loginId) {
 		return getSecurityInfoDao().getSessionTimeout(splicingKeySession(loginId));
 	}
 	...
 	public long getTokenSessionTimeoutByTokenValue(String tokenValue) {
 		return getSecurityInfoDao().getSessionTimeout(splicingKeyTokenSession(tokenValue));
 	}
 	...
 	public long getTokenActivityTimeoutByToken(String tokenValue) {
 		// 如果token为null , 则返回 -2
 		if(tokenValue == null) {
 			return SecurityInfoDao.NOT_VALUE_EXPIRE;
 		}
 		// 如果设置了永不过期, 则返回 -1 
 		if(isOpenActivityCheck() == false) {
 			return SecurityInfoDao.NEVER_EXPIRE;
 		}
 		// ------ 开始查询 
 		// 获取相关数据 
 		String keyLastActivityTime = splicingKeyLastActivityTime(tokenValue);
 		String lastActivityTimeString = getSecurityInfoDao().get(keyLastActivityTime);
 		// 查不到，返回-2 
 	...
 	public void renewTimeout(String tokenValue, long timeout) {
		...
		SecurityInfoDao dao = getSecurityInfoDao();
 		
 		// 续期 Token 有效期 
 		dao.updateTimeout(splicingKeyTokenValue(tokenValue), timeout);

 		// 续期 Token-Session 有效期 
		SecurityInfoSession tokenSession = getTokenSessionByToken(tokenValue, false);
	...
	public List<String> getRoleList(Object loginId) {
		return SecurityManager.getStpInterface().getRoleList(loginId, loginType);
	}
	...
	public List<String> getPermissionList(Object loginId) {
		return SecurityManager.getStpInterface().getPermissionList(loginId, loginType);
	}
	...
	public String getLoginDevice() {
		...
		SecurityInfoSession session = getSessionByLoginId(getLoginIdDefaultNull(), false);
		if(session == null) {
			return null;
		}
		// 遍历解析 
	...
	public List<String> searchTokenValue(String keyword, int start, int size) {
		return getSecurityInfoDao().searchData(splicingKeyTokenValue(""), keyword, start, size);
	}
	...
	public List<String> searchSessionId(String keyword, int start, int size) {
		return getSecurityInfoDao().searchData(splicingKeySession(""), keyword, start, size);
	}
	...
	public List<String> searchTokenSessionId(String keyword, int start, int size) {
		return getSecurityInfoDao().searchData(splicingKeyTokenSession(""), keyword, start, size);
	}
	 ...
	 * @param at 注解对象 
	 */
	public void checkByAnnotation(CheckLogin at) {
		this.checkLogin();
	}
	 ...
	 * @param at 注解对象 
	 */
	public void checkByAnnotation(CheckRole at) {
	...
	/**
	 * 根据注解(@CheckPermission)鉴权
	 * @param at 注解对象 
	 */
	public void checkByAnnotation(CheckPermission at) {
	 ...
	 * @param at 注解对象 
	 */
	public void checkByAnnotation(CheckSafe at) {
		this.checkSafe();
	}
	...
	public void disable(Object loginId, long disableTime) {
		// 标注为已被封禁 
		getSecurityInfoDao().set(splicingKeyDisable(loginId), DisableLoginException.BE_VALUE, disableTime);
 		
 		// $$ 通知监听器 
 		SecurityManager.getSecurityListener().doDisable(loginType, loginId, disableTime);
	}
	...
	public boolean isDisable(Object loginId) {
		return getSecurityInfoDao().get(splicingKeyDisable(loginId)) != null;
	}
	...
	public long getDisableTime(Object loginId) {
		return getSecurityInfoDao().getTimeout(splicingKeyDisable(loginId));
	}
	...
	public void untieDisable(Object loginId) {
		getSecurityInfoDao().delete(splicingKeyDisable(loginId));
		
 		// $$ 通知监听器 
 		SecurityManager.getSecurityListener().doUntieDisable(loginType, loginId);
	}
	...
	public void switchTo(Object loginId) {
		SecurityHolder.getStorage().set(splicingKeySwitch(), loginId);
	}
	 ...
	 */
	public void switchTo(Object loginId, CoreFunction function) {
		try {
			switchTo(loginId);
			function.run();
		} catch (Exception e) {
	...
	public void openSafe(long safeTime) {
		...
		getTokenSession().set(CoreConsts.SAFE_AUTH_SAVE_KEY, eff);
	}
	...
	public boolean isSafe() {
		long eff = getTokenSession().get(CoreConsts.SAFE_AUTH_SAVE_KEY, 0L);
	...
	public void closeSafe() {
		getTokenSession().delete(CoreConsts.SAFE_AUTH_SAVE_KEY);
	}
	...
	public String splicingKeySwitch() {
		return CoreConsts.SWITCH_TO_SAVE_KEY + loginType;
	}
...
//		return CoreConsts.JUST_CREATED_SAVE_KEY + loginType;
		return CoreConsts.JUST_CREATED;
	}
	...
	public SecurityConfig getConfig() {
		// 为什么再次代理一层? 为某些极端业务场景下[需要不同LspLogic不同配置]提供便利 
 	...
 	public boolean isOpenActivityCheck() {
 		return getConfig().getActivityTimeout() != SecurityInfoDao.NEVER_EXPIRE;
 	}
	...
	public SecurityInfoDao getSecurityInfoDao() {
		return SecurityManager.getSecurityInfoDao();
	}
	...
	public boolean hasElement(List<String> list, String element) {
		return SecurityInfoStrategy.me.hasElement.apply(list, element);
	}
